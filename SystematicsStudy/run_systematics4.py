...
if __name__ == '__main__':
    ...
    tag_par_names = ['SignalTagging.bins_over_threshold', 'SignalTagging.threshold','SignalTagging.signal_length']
    tag_ranges       = [[1,2,3], [11.7,19.8,27.3], [200,375,500]]

    decomp_par_names = [ 'WaveformDecomposition.threshold','WaveformDecomposition.fwhm']
    ...
    par_names = tag_par_names + decomp_par_names + miptime_par_names + result_par_names
    par       = [0] * len(par_names)

    n = 1
    nges = 1
    for r in (tag_ranges + decomp_ranges+miptime_ranges):
        nges =  nges*len(r)
    ...
    for par[-len(result_par_names) - len(miptime_par_names) -len(decomp_par_names) - 3] in tag_ranges[-3]:
        change_parameter(args.cfile, tag_par_names[-3],  par[-len(result_par_names) - len(miptime_par_names) -len(decomp_par_names) -3])

        for par[-len(result_par_names) - len(miptime_par_names) -len(decomp_par_names) - 2] in tag_ranges[-2]:
            change_parameter(args.cfile, tag_par_names[-2],  par[-len(result_par_names) - len(miptime_par_names) -len(decomp_par_names) -2])

            for par[-len(result_par_names) - len(miptime_par_names) -len(decomp_par_names) - 1] in tag_ranges[-1]:
                change_parameter(args.cfile, tag_par_names[-1],  par[-len(result_par_names) - len(miptime_par_names) -len(decomp_par_names) -1])

                for i in range(len(par_names)):
                    print("Name: " + str(par_names[i]) + ' Value: ' + str(par[i]))
                print('NOW RUNNING 6')
                os.system(cmd + ' --tasks 6')

                for par[-len(result_par_names) - len(miptime_par_names)- 2] in decomp_ranges[-2]:
                    change_parameter(args.cfile, decomp_par_names[-2],  par[-len(result_par_names) - len(miptime_par_names)-2])

                    for par[-len(result_par_names) - len(miptime_par_names)- 1] in decomp_ranges[-1]:
                        change_parameter(args.cfile, decomp_par_names[-1],  par[-len(result_par_names) - len(miptime_par_names)-1])
                        ...
                        for par[-len(result_par_names) - 4] in miptime_ranges[-4]:
                            ...
                            for par[-len(result_par_names) - 3] in miptime_ranges[-3]:
                                ...
                                if(par[-len(result_par_names) - 3] == 'constant fraction'):
                                    ...
                                    for par[-len(result_par_names) -2] in miptime_ranges[-2]:

                                        change_parameter(args.cfile, miptime_par_names[-2],  par[-len(result_par_names) -2])
                                        ...
                                        rfile = TFile( args.input + '/Results/SystematicsStudy/run_'+str(runnr)+'_systematics_ph1v5.root', 'OPEN')
                                ...
                                elif(par[-len(result_par_names) - 3] == 'pe hit time'):

                                    for par[-len(result_par_names) -1] in miptime_ranges[-1]:

                                        change_parameter(args.cfile, miptime_par_names[-1],  par[-len(result_par_names) -1])
